...
export async function generateMetadata(
  props: PageProps<"/[idioma]/terminos">,
): Promise<Metadata> {
  const { idioma } = await props.params;
  if (!esIdioma(idioma)) return {};
  const p = paginaLegal(idioma, "terminos");
  return {
    title: p.titulo,
    description: p.descripcion,
    alternates: alternativas({ tipo: "terminos" }, idioma),
  };
}

export default async function Terminos(props: PageProps<"/[idioma]/terminos">) {
  const { idioma } = await props.params;
        ...
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
          <Migas
            migas={[
              ...
              { nombre: p.titulo, pagina: { tipo: "terminos" } },
            ]}
            idioma={idioma}
          />
