...
export async function generateMetadata(
  props: PageProps<"/[idioma]/aviso-legal">,
): Promise<Metadata> {
  const { idioma } = await props.params;
  if (!esIdioma(idioma)) return {};
  const p = paginaLegal(idioma, "avisoLegal");
  return {
    title: p.titulo,
    description: p.descripcion,
    alternates: alternativas({ tipo: "avisoLegal" }, idioma),
  };
}

export default async function AvisoLegal(props: PageProps<"/[idioma]/aviso-legal">) {
  const { idioma } = await props.params;
        ...
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
          <Migas
            migas={[
              ...
              { nombre: p.titulo, pagina: { tipo: "avisoLegal" } },
            ]}
            idioma={idioma}
          />
